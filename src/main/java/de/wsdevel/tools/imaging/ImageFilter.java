 ...
 * @version $Author: sweissTFH $ -- $Revision: 1.6 $ -- $Date: 2006-09-01
 *          14:42:09 $
 * 
 * <br>
...
public final class ImageFilter {
    ...
    private static final Log LOG = LogFactory.getLog(ImageFilter.class);
    ...
    /**
     * createPNGBytesForComponent.
     * 
     * @param c
     * @return
     */
    public static synchronized byte[] createPNGBytesForComponent(
	    final Component c) {
	...
	c.paint(g);
	g.dispose();
	return writeBufferedImageAsPNGAsByteArray(image);
    }
	...
	final int destWidth = (int) (image.getWidth() * widthScaleFactor);
	final int destHeight = (int) (image.getHeight() * heightScaleFactor);

	final BufferedImage resizedImage = ImageFilter.GC
	...
	final BufferedImage thumbnail = ImageFilter.GC.createCompatibleImage(
		newMaxWidth, newMaxHeight, Transparency.TRANSLUCENT);
     ...
     * @param newMaxHeight
     *            <code>int</code>
     * @return {@link BufferedImage} or <code>null</code> if image could not be
     *         resized!
     * @throws IOException
	...
	final BufferedImage orgImage = ImageIO.read(imageToResize);
	if (orgImage == null) {
	    return null;
	}
     ...
     * @param bi
     *            {@link BufferedImage}
     * @return <code>byte</code>[] or <code>null</code> if an error occurred!
     */
    public static byte[] writeBufferedImageAsJPGAsByteArray(
	    final BufferedImage bi) {
	return writeBufferedImageToByteArray(bi, "jpg");
    }
     ...
     * @return <code>byte</code>[] or <code>null</code> if an error occurred!
     */
    public static byte[] writeBufferedImageAsPNGAsByteArray(
	    final BufferedImage bi) {
	return writeBufferedImageToByteArray(bi, "png");
    }

    /**
     * writeBufferedImageToByteArray.
     * 
     * @param bi
     * @param formatName
     * @return
     */
    private static byte[] writeBufferedImageToByteArray(final BufferedImage bi,
...
/*
 * $Log: ImageFilter.java,v $ Revision 1.6 2006-09-01 14:42:09 sweissTFH added
 * scaling functionality
 * 
 * Revision 1.5 2006/06/10 17:57:30 sweissTFH clean up
 * 
 * Revision 1.4 2006/06/10 16:30:14 sweissTFH clean up
 * 
 * Revision 1.3 2006/06/02 12:18:50 sweissTFH improvements
 * 
