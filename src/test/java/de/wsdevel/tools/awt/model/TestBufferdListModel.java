...
public class TestBufferdListModel extends ListModelTest {
	...
	BufferedListModel buffered = new BufferedListModel(
		this.list.getListModel(), ObserverType.KEEP_IN_THREAD);
	buffered.addListDataListener(this.ldl);

	assertEquals("incorrect number of elements", 5, buffered.getSize());
	assertEquals("incorrect element in buffer", "A",
		buffered.getElementAt(0));
	assertEquals("incorrect element in buffer", "B",
		buffered.getElementAt(1));
	assertEquals("incorrect element in buffer", "C",
		buffered.getElementAt(2));
	assertEquals("incorrect element in buffer", "D",
		buffered.getElementAt(3));
	assertEquals("incorrect element in buffer", "E",
		buffered.getElementAt(4));
	...
	assertEquals("incorrect element in buffer", "F",
		buffered.getElementAt(5));
	...
	assertEquals("incorrect element in buffer", "F",
		buffered.getElementAt(4));
	assertEquals("incorrect element in buffer", "Q",
		buffered.getElementAt(1));
...
/*
 * $Log: TestBufferdListModel.java,v $ Revision 1.4 2006-07-21 11:11:00
 * sweissTFH some comment fixes
 * 
