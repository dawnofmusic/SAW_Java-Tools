...
public class ListModelTest extends TestCase {
    ...
    void checkEvent(final Object source, final int index0, final int index1,
        ...
        if (this.lastEventFired != null) {
            assertEquals("Correct source in event", source, this.lastEventFired
                    .getSource());
            assertEquals("invalid indexes", index0, this.lastEventFired.getIndex0());
            assertEquals("invalid indexes", index1, this.lastEventFired.getIndex1());
            assertEquals("invalid event type", type, this.lastEventFired.getType());
        } else {
    ...
    final void checkEventCalls(final int contentCount, final int addedCount,
        ...
        assertTrue("event should not occured",
                this.contentsChangedCalled == contentCount);
        assertTrue("event should not occured",
                this.intervallAddedCalled == addedCount);
        assertTrue("event should not occured",
                this.intervallRemovedCalled == removedCount);
    }

}
/*
 * $Log: ListModelTest.java,v $
 * Revision 1.3  2007-07-27 15:01:08  sweiss
 * added test config
 *
